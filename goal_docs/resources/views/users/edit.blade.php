...
<head>
    ...
    <title>Edit User - GoalDocs</title>
    <meta name="description" content="Edit user information and position" />
                            ...
                            <div class="col-12 col-lg-8">
                                <div class="card mb-4">
                                    <div class="card-header">
                                        ...
                                        <small class="text-muted">Update user information and position</small>
                                    </div>
                                    <div class="card-body">
                                        <form action="{{ route('users.update', $user) }}" method="POST">
                                            ...
                                            <div class="row">
                                                            ...
                                                            <optgroup label="{{ $department->name }}">
                                                                @foreach($department->activePositions as $position)
                                                                    <option value="{{ $position->id }}" 
                                                                            {{ old('position_id', $currentPositionId) == $position->id ? 'selected' : '' }}>
                                                ...
                                                <div class="col-md-4 mb-3">
                                                    ...
                                                    @php
                                                        $currentStartDate = $user->positions->where('pivot.is_primary', true)->first()->pivot->start_date ?? null;
                                                    @endphp
                                                    <input type="date" class="form-control @error('start_date') is-invalid @enderror" 
                                                           id="start_date" name="start_date" value="{{ old('start_date', $currentStartDate) }}">
                                                    @error('start_date')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                </div>
                                                 ...
                                                 <div class="card bg-warning-subtle border-warning">
                                                     <div class="card-body py-3">
                                                         ...
                                                         <small class="text-muted">
                                                             ...
                                                             Contact an administrator to change admin privileges for this user.
                                                         </small>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="text-center mb-3">
                                            <div id="user-avatar" class="mx-auto mb-2" style="width: 60px; height: 60px; border-radius: 50%; background: {{ '#' . substr(md5($user->email), 0, 6) }}; display: flex; align-items: center; justify-content: center; color: white; font-weight: 600; font-size: 18px;">
                                                {{ strtoupper(substr($user->first_name, 0, 1) . substr($user->last_name, 0, 1)) }}
                                            </div>
                                            <h6 id="user-name" class="mb-0">{{ $user->first_name }} {{ $user->last_name }}</h6>
                                            @php $currentPosition = $user->positions->where('pivot.is_primary', true)->first() @endphp
                                            <small id="user-position" class="text-muted">
                                                @if($currentPosition)
                                                    {{ $currentPosition->name }} ({{ ucfirst($currentPosition->level) }} Level)
                                                @else
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <div id="user-email" class="mb-1">{{ $user->email }}</div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            @if($user->is_admin)
                                                <span id="admin-badge" class="badge bg-label-danger">Administrator</span>
                                            @else
                                        ...
                                        <div class="alert alert-warning">
                                            ...
                                            <small>Changes to user information will be saved immediately. A notification will be sent to the user about any position changes.</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        $(document).ready(function() {
            ...
            function updatePreview() {
                ...
                if (positionId && positionText) {
                    ...
                    $('#user-position').text('No position assigned');
                }
